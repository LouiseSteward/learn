...
then
    ...
    then
        rsync -a "$HOME/cache/_site/" "$SSH_LIVE":"$DESTINATIONPATH_LIVE"
    else
        lftp -c "open -u $FTP_USER_LIVE,$FTP_PASSWORD_LIVE $FTP_HOST_LIVE; set ssl:verify-certificate no; mirror -R '$HOME/cache/_site/' '$DESTINATIONPATH_LIVE'"
    fi
    ...
    then
        ...
        then
            rsync -a "$HOME/cache/_site/" "$SSH_PREVIEWS":"$DESTINATIONPATH_PREVIEWS/$tag"
        else
            # If your server can't handle a queue of FTP commands,
            # you may need to remove `set ftp:sync-mode false;`
            lftp -c "open -u $FTP_USER_PREVIEWS,$FTP_PASSWORD_PREVIEWS $FTP_HOST_PREVIEWS; set ssl:verify-certificate no; set ftp:sync-mode false; mirror -R '$HOME/cache/_site/' '$DESTINATIONPATH_PREVIEWS/$tag'"
        fi
    else
        if [[ $DEPLOY_METHOD_STAGING == 'SSH' ]]
        then
            rsync -a "$HOME/cache/_site/" "$SSH_STAGING":"$DESTINATIONPATH_STAGING"
        else
            # If your server can't handle a queue of FTP commands,
            # you may need to remove `set ftp:sync-mode false;`
            lftp -c "open -u $FTP_USER_STAGING,$FTP_PASSWORD_STAGING $FTP_HOST_STAGING; set ssl:verify-certificate no; set ftp:sync-mode false; mirror -R '$HOME/cache/_site/' '$DESTINATIONPATH_STAGING'"
        fi
    fi
