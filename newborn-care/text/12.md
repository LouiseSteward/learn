...
1. {:.allow-break}Mild conjunctivitis can usually be treated by cleaning the eye with saline or warm water if the lashes become sticky. A local antibiotic is frequently not needed. However, if the infection does not recover in a few days, tetracycline or chloromycetin ointment should be used 6 hourly for 5 days. Tetracycline, chloromycetin and erythromycin ointment will kill Gonococcus but only erythromycin and tetracycline will treat Chlamydia.
2. {:.allow-break}Moderate conjunctivitis should be treated by cleaning the eye and then instilling tetracycline or chloromycetin ointment 3 hourly or more frequently if needed. Usually 5 days treatment is needed.
3. {:.allow-break}Severe conjunctivitis is a medical emergency as it can lead to blindness if not promptly and efficiently treated. The infection is usually due to the Gonococcus and treatment consists of irrigating the eye and giving intramuscular ceftriaxone.
...
Note
In the first week of life the normal values are:
...
{% include figure
   markdown="| Birth weight     | Daily dosage                                    | Quantity               |
|------------------+-------------------------------------------------+------------------------|
...
| More than 2.5 kg | Birth to 6 weeks: 15 mg                          | 1.5 ml                 |"
   caption="*Table 12-1: NVP dosing guidelines for newborns: NVP syrup 10mg/ml*"
   description="*Table 12-1: NVP dosing guidelines for newborns: NVP syrup 10mg/ml*"
   class="wide"
%}
...
{% include youtube id="XezF_TOMU8E" class="non-printing" %}
...
{% include youtube id="9tykrHSJbhg" class="non-printing" %}
...
{% include youtube id="O4W7WW7xNkY" class="non-printing" %}
...
{% include youtube id="9nkRuF04Y8c" class="non-printing" %}
